...
# sir model
sir_R0 <- odin.dust::odin_dust("sir_R0_example/sir_R0.R") # standard SIR model
...
source("sir_R0_example/compare_sir_R0.R") # observation process could also appear here
source("sir_R0_example/index_sir_R0.R")

#parameters
pars <- list(R0 = 1.0, gamma = 0.1, S0 = pop_size) #leave others as defaults
...
matplot(h["t", 1, ], t(h["I", , ]), type = "l", col = "#00000011", 
        xlab = "Day", ylab = "Number of infected (I)", las = 1) #now plotting underlying infections

# now for the fitting
priors <- list(
  mcstate::pmcmc_parameter("R0", 2.0, min = 0),
...
# plot result
pars <- list(R0 = samples$pars[n_steps_in,1], gamma = samples$pars[n_steps_in,2])
filter$run(pars, save_history = TRUE)
h <- filter$history()
