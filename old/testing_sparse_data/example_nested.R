...
sir <- odin.dust::odin_dust("testing_sparse_data/sir_temp_new2.R")
...
filter <- particle_filter$new(data,  model = sir, n_particles=100, compare = compare_nb, index = index)
...
par(mfrow=c(1,2))
matplot(x=c(1:(t_pts+1)),y=t(samples$trajectories$state["cases",1,c(burnin:n_steps_in),]),type="p",pch=16,cex=0.25,col=1,
        ylim=c(0,max(c(samples$trajectories$state["cases",1,c(burnin:n_steps_in),],df_tidy$cases))),xlab="Day",ylab="Cases")
matplot(x=df_tidy$day[c(1:t_pts)],y=df_tidy$cases[df_tidy$population=="A"],type="l",col=2,lwd=2.0,add=TRUE)
legend("topleft",c("Modelled", "Observed"),col=c(1,2),pch=c(16,NA),lty=c(0,1))
title("A")
matplot(x=c(1:(t_pts+1)),y=t(samples$trajectories$state["cases",2,c(burnin:n_steps_in),]),type="p",pch=16,cex=0.25,col=1,
        ylim=c(0,max(c(samples$trajectories$state["cases",2,c(burnin:n_steps_in),],df_tidy$cases))),xlab="Day",ylab="Cases")
matplot(x=df_tidy$day[c(1:t_pts)],y=df_tidy$cases[df_tidy$population=="B"],type="l",col=2,lwd=2.0,add=TRUE)
legend("topleft",c("Modelled", "Observed"),col=c(1,2),pch=c(16,NA),lty=c(0,1))
title("B")
par(mfrow=c(1,1))
