...
library(odin.dust)
model <- odin.dust::odin_dust("multiple_genera_new/seir_multiple_genera_new.R")

#Set up parameters--------------------------------------------------------------
dt=1 #Time increment in days
c_beta_temp=matrix(1e-5,ncol=2,nrow=2) #Coefficients used to calculate temperature component of beta parameter
c_beta_precip=matrix(1e-6,ncol=2,nrow=2) #Coefficients used to calculate precipitation component of beta parameter
c_FOI_in_temp=c(1e-5,1e-5) #Coefficients used to calculate temperature component of FOI_in parameter
c_FOI_in_precip=c(1e-6,1e-6) #Coefficients used to calculate precipitation component of FOI_in parameter
...
output=array(NA,dim=c(pars$n_t_pts,n_particles,1+(7*pars$n_gen)))
...
FOI=output[,1,1+2+c(1:pars$n_gen)] #Total calculated force of infection at each time point
S=output[,1,1+pars$n_gen+c(1:pars$n_gen)] #Susceptible population by genus at each time point
E=output[,1,1+(2*pars$n_gen)+c(1:pars$n_gen)] #Exposed population by genus at each time point
I=output[,1,1+(3*pars$n_gen)+c(1:pars$n_gen)] #Infectious population by genus at each time point
R=output[,1,1+(4*pars$n_gen)+c(1:pars$n_gen)] #Recovered population by genus at each time point
C=output[,1,1+(5*pars$n_gen)+c(1:pars$n_gen)] #New infections by genus at each time point
D=output[,1,1+(6*pars$n_gen)+c(1:pars$n_gen)] #Deaths by genus at each time point
