# Human SEIRV model, written in format for compilation by odin.dust package
# This is a version of the model in the YEP package, modified to accept spillover force of infection
# and R0 data as vectors rather than scalars, so that these quantities can vary with time
# This code is best left unchanged to maintain agreement with YEP
...
FOI_spillover[] <- user()    #Spillover force of infection (per day) for each time point
R0[] <- user()               #Basic reproduction number for human-human transmission for each time point
...
n_pts <- n_years*(365.0/dt) #Total number of time points
...
rate1 <- dt/(t_incubation+t_latent) #Rate of transfer from E to I
rate2 <- dt/t_infectious            #Rate of transfer from I to R

#Time varying non-output calculated values--------------------------------------
beta <- (R0[step]*dt)/t_infectious                                      #Daily exposure rate
FOI_sum <-  min(FOI_max,beta*(sum(I)/P_tot) + (FOI_spillover[step]*dt)) #Total force of infection
