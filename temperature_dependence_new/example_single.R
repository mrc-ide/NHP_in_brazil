...
plot(samples$probabilities[,3])

matplot(x=c(1:101),y=samples$trajectories$state["cases",n_steps_in,],type="p",pch=1,col=1,
        ylim=c(0,max(c(samples$trajectories$state["cases",n_steps_in,],df_tidy$cases))),xlab="Day",ylab="Cases")
matplot(x=df_tidy$day,y=df_tidy$cases,type="p",pch=1,col=2,add=TRUE)

parameters_out=list(log_gamma=samples$pars[,1],log_temp_scale=samples$pars[,2])

matplot(x=c(1:n_steps_in),y=parameters_out$log_gamma,type="l",col=1,cex=0.5,xlab="Iteration",ylab="Value",
...
matplot(x=c(1:n_steps_in),y=parameters_out$log_temp_scale,type="l",col=2,cex=0.5,add=TRUE)
